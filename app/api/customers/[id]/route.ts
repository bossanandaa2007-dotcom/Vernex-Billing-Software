...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const customer = await db.customer.findFirst({ where: { id: params.id, businessId: ctx.businessId }, include: { transactions: { where: { isComplete: true, businessId: ctx.businessId }, orderBy: { completedAt: 'desc' } } } });
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const customer = await db.customer.findFirst({ where: { id: params.id, businessId: ctx.businessId } });
    if (!customer) return NextResponse.json({ error: 'Customer not found.' }, { status: 404 });
    const updated = await db.customer.update({ where: { id: params.id }, data: { ...value, email: value.email || null, address: value.address || null, taxId: value.taxId || null, country: value.country || null, notes: value.notes || null } });
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const customer = await db.customer.findFirst({ where: { id: params.id, businessId: ctx.businessId } });
    if (!customer) return NextResponse.json({ error: 'Customer not found.' }, { status: 404 });
    const updated = await db.customer.update({ where: { id: params.id }, data: { isActive: false } });
