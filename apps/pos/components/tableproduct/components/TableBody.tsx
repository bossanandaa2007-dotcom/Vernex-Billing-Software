...
const TableBodyProduct: React.FC<TableBodyProductProps> = ({ data, currency }) => {
        ...
        : // Render product data if not loading
          productData.map((item) => (
            <TableRow key={item.id}>
              ...
              <TableCell className="pl-5">{formatMoney(item.sellprice, currency)}</TableCell>
              ...
              <TableCell className="hidden md:table-cell pl-4">
                {formatMoney(item.productstock.price, currency)}
              </TableCell>
