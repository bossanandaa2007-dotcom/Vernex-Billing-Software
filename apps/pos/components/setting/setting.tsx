          ...
          <div className="grid gap-6">
            ...
            <div className="rounded-xl border border-vernex-border bg-white p-5 shadow-sm dark:border-[#1E335F] dark:bg-vernex-navy">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {enabledModules.map((module: string) => <span key={module} className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-300">{module.replaceAll('_', ' ')}</span>)}
              </div>
            </div>
          </div>
        </div>
