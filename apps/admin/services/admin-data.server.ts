...
export async function getDashboard(): Promise<DashboardSnapshot> {
  const supabase = await client();
  const { rows: businesses } = await listBusinesses({ pageSize: 500 });
  const [
    ...
    supabase.from('Transaction').select('id,businessId,totalAmount,completedAt').eq('isComplete', true),
    supabase.from('ProductStock').select('*', { count: 'exact', head: true }),
    supabase.from('Customer').select('*', { count: 'exact', head: true }),
    supabase.from('StaffProfile').select('*', { count: 'exact', head: true }),
    listAuditLogs(),
  ]);
