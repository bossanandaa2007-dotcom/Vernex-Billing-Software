...
const navItems = [
  ...
  { href: '/trials', label: 'Trials', icon: Activity },
...
export function AdminShell({
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] dark:bg-[#071327]">
      ...
      {mobileOpen && <div className="fixed inset-0 z-50 lg:hidden"><button aria-label="Close navigation" className="absolute inset-0 bg-slate-950/50" onClick={() => setMobileOpen(false)} /><aside className="relative h-full w-72 shadow-2xl">{sidebar}<button aria-label="Close navigation" onClick={() => setMobileOpen(false)} className="absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-md text-white hover:bg-white/10"><X className="h-5 w-5" /></button></aside></div>}
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 flex h-18 items-center gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur md:px-6 dark:border-slate-800 dark:bg-slate-950/95">
          ...
          <div className="relative ml-auto w-full max-w-md">
            ...
            {searchOpen && query.trim().length >= 2 && <div className="absolute right-0 top-12 w-full min-w-80 rounded-lg border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900">
              ...
              {searching ? <p className="p-4 text-sm text-slate-500">Searching...</p> : results.businesses.length || results.users.length ? <div className="max-h-80 overflow-auto">
                {results.businesses.map((business) => <Link key={business.id} href={`/businesses/${business.id}`} className="flex items-center gap-3 rounded-md p-2 hover:bg-slate-50 dark:hover:bg-slate-800"><Building2 className="h-4 w-4 text-amber-600" /><span><b className="block text-sm">{business.name}</b><span className="text-xs text-slate-500">{business.ownerEmail}</span></span></Link>)}
                {results.users.map((user) => <Link key={user.id} href={`/users?search=${encodeURIComponent(user.email)}`} className="flex items-center gap-3 rounded-md p-2 hover:bg-slate-50 dark:hover:bg-slate-800"><Users className="h-4 w-4 text-blue-600" /><span><b className="block text-sm">{user.name}</b><span className="text-xs text-slate-500">{user.email}</span></span></Link>)}
