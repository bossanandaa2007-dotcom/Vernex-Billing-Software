...
export function TrialActions({ id }: { id: string }) {
  ...
  async function run(action: 'extend' | 'activate' | 'expire') {
    setLoading(action);
    setError('');
    const response = await fetch(`/api/admin/businesses/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(action === 'extend' ? { action, days: 14 } : { action }) });
