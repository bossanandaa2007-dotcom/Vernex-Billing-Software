...
import { MODULE_GROUPS, MODULE_KEYS, type ModuleKey } from '@/lib/modules';
...
export function BusinessModulesForm({ businessId }: { businessId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/businesses/${businessId}/modules`, { cache: 'no-store' })
      .then(async (response) => {
  ...
  async function save() {
    ...
    const response = await fetch(`/api/admin/businesses/${businessId}/modules`, {
