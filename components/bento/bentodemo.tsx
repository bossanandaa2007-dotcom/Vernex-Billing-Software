      ...
      <div className="rounded-xl border border-vernex-border bg-white p-5 shadow-sm dark:border-[#1E335F] dark:bg-vernex-navy">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-black text-vernex-navy dark:text-white">{activePeriod.title}</h2>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex rounded-lg border border-vernex-border bg-vernex-surface p-1 dark:border-[#1E335F] dark:bg-vernex-dark">
            ...
            {viewModes.map((item) => (
              <Button key={item.value} size="sm" variant={view === item.value ? 'default' : 'outline'} onClick={() => setView(item.value)}>
                {item.label}
              </Button>
            ))}
            <Button size="icon" variant="outline" title="Refresh dashboard" onClick={load} disabled={loading}>
