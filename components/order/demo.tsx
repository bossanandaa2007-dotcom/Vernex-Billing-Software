...
export function Orders() {
  ...
  const addProduct = async (productId: string, quantity = 1) => {
    ...
    try {
      ...
      toast.error(typeof message === 'string' ? message : 'Unable to add product.');
    } finally {
      setMutating(false);
      searchRef.current?.focus();
    }
  ...
  const updateQuantity = async (line: TransactionData, quantity: number) => {
    ...
    try {
      ...
      const message = axios.isAxiosError(error) ? error.response?.data?.error : null;
      toast.error(typeof message === 'string' ? message : 'Unable to update quantity.');
    } finally {
      setMutating(false);
    }
  };
        ...
        <Card className="overflow-hidden border-vernex-border/80 shadow-sm">
          <CardHeader className="space-y-4 bg-white/80 pb-4 dark:bg-vernex-navy/60">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button variant="outline" size="icon" onClick={() => fetchProducts()} disabled={loadingProducts}>
          ...
          <CardContent className="min-h-[520px] overflow-auto p-4">
            {loadingProducts ? <ProductSkeleton /> : filteredProducts.length ? (
              ...
              <EmptyState title="No products found" description="Try another search, category, or barcode value." />
            )}
          </CardContent>
        </Card>
      </section>

      <div className="fixed bottom-4 left-4 right-4 z-20 md:left-[calc(220px+1rem)] lg:left-[calc(280px+1rem)]">
        <div className="mx-auto flex max-w-7xl items-center justify-between rounded-2xl border border-vernex-border bg-white p-3 shadow-xl dark:border-[#1E335F] dark:bg-vernex-navy">
          ...
          <Button className="h-11 rounded-xl bg-emerald-600 px-8 hover:bg-emerald-700" disabled={!transactionData.length} onClick={() => setActiveOverlay('bill')}>
        ...
        <Card className="h-full overflow-hidden border-vernex-border/80 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between gap-3 bg-white/80 pb-4 dark:bg-vernex-navy/60">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => toast.info('Bill is held on this device. Resume by returning to POS.')} disabled={!transactionId}>
                <Clock3 className="mr-2 h-4 w-4" /> Hold
              </Button>
              <Button variant="destructive" size="sm" onClick={clearBill} disabled={!transactionId || mutating}>
                <Trash2 className="mr-2 h-4 w-4" /> Clear
              </Button>
              <Button variant="ghost" size="icon" onClick={() => setActiveOverlay(null)}>
                <X className="h-4 w-4" />
              </Button>
            </div>
          </CardHeader>
          <CardContent className="p-0">
                    ...
                    return (
                      <tr key={line.id} className="border-t border-vernex-border bg-white transition hover:bg-vernex-surface/70 dark:border-[#1E335F] dark:bg-vernex-navy/70 dark:hover:bg-vernex-navy">
                        ...
                        <td className="px-4 py-3 text-right">
                          <Button variant="ghost" size="icon" onClick={() => removeLine(line)}>
                            <X className="h-4 w-4" />
                          </Button>
                        </td>
                      </tr>
            ...
            {!transactionData.length && (
              <EmptyState title="Your bill is empty" description="Search or scan a product to begin this customer checkout." />
            )}
          </CardContent>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-[1fr_auto] gap-2">
                ...
                <Button variant="outline" size="icon" title="Quick customer creation">
            ...
            <div className="grid gap-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Button variant="outline" onClick={() => window.print()}><FileDown className="mr-2 h-4 w-4" /> PDF</Button>
                <Button variant="outline" onClick={() => toast.success('Draft saved on this device.')}><CreditCard className="mr-2 h-4 w-4" /> Draft</Button>
                <Button variant="outline" onClick={() => toast.info('Held bill can be resumed from this POS device.')}><History className="mr-2 h-4 w-4" /> Hold</Button>
              </div>
            ...
            <div className="rounded-xl border border-dashed border-vernex-border p-3 text-xs text-vernex-muted dark:border-[#1E335F] dark:text-slate-400">
              Shortcuts: F2 search, Enter add first result, F4 payment, Ctrl+Backspace clear.
            </div>
          </CardContent>
        </Card>
      </aside>
...
function EmptyState({ title, description }: { title: string; description: string }) {
  return (
    <div className="flex min-h-48 flex-col items-center justify-center px-6 py-12 text-center">
      <div className="rounded-full bg-vernex-gold/10 p-4 text-vernex-gold"><PackageSearch className="h-7 w-7" /></div>
      <div className="mt-4 font-semibold">{title}</div>
      <p className="mt-1 max-w-sm text-sm text-vernex-muted dark:text-slate-400">{description}</p>
    </div>
  );
}
