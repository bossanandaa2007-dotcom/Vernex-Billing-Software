...
export function ReturnPanel({ transactionId, items, returns, onComplete }: { transactionId: string; items: Item[]; returns: PreviousReturn[]; onComplete: () => void }) {
  ...
  const [quantities, setQuantities] = useState<Record<string, number>>({}); const [method, setMethod] = useState('CASH'); const [reason, setReason] = useState('Customer return');
  const submit = async () => { if (isBlocked) return toast.error(expiredMessage); const selected = items.map((item) => ({ saleLineId: item.id, quantity: quantities[item.id] ?? 0 })).filter((item) => item.quantity > 0); if (!selected.length) return toast.error('Select at least one return quantity.'); try { await axios.post('/api/returns', { transactionId, refundMethod: method, reason, items: selected }); toast.success('Return completed and stock restored.'); onComplete(); } catch (error) { toast.error(axios.isAxiosError(error) ? error.response?.data?.error ?? 'Return failed.' : 'Return failed.'); } };
  return <Card><CardHeader><CardTitle>Return / Refund</CardTitle></CardHeader><CardContent className="space-y-4">
    ...
    <Button onClick={submit} disabled={isBlocked}>Process Return</Button>
  </CardContent></Card>;
}
