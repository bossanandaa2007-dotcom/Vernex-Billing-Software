      ...
      const fetchProductStocks = async () => {
        try {
          ...
          const [productsResponse, shopResponse] = await Promise.all([
            axios.get<Data[]>('/api/storage'),
            axios.get('/api/shopdata'),
          ]);
          setProductStocks(productsResponse.data);
          setCurrency(shopResponse.data.data?.currency ?? 'INR');
        } catch (error) {
                ...
                <Input
                  ...
                  value={
                    selectedResult.Product && selectedResult.Product.length > 0
                      ? formatMoney(selectedResult.Product[0].sellprice, currency)
                      : 'Unavailable'
                  }
                  readOnly
                />
